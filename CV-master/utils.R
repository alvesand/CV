...
generate_latex_table <- function(data, centered, pad) {
  # Calculate maximum width for each column
  col_widths <- apply(data, 2, function(col) 
    paste(round(max(nchar(as.character(col))*0.22)+pad), 
          "cm", sep = ""))    # Add some padding
  
  if (centered == T){cat("\\begin{tabular}{|", paste(rep(">{\\centering\\arraybackslash}p{"  , ncol(data)), col_widths, "} |", collapse = ""), "}\n")}
  else{cat("\\begin{tabular}{|", paste(rep("p{"  , ncol(data)), col_widths, "} |", collapse = ""), "}\n")}
  cat("\\hline\n")
  cat("\\textbf{", paste(names(data), collapse = "} & \\textbf{"), "} \\\\\n")  # Bold column names
  cat("\\hline\n")
  for (row in 1:nrow(data)) {
    cat(paste(data[row, ], collapse = " & "), " \\\\\n")
  }
  cat("\\hline\n")
  cat("\\end{tabular}\n")
}
